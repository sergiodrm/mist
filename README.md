...
![gif](/docs/demo_bistro.png)
![gif](/docs/demo_sponza.gif)
...
### Generate VS project
Execute `GenerateProject.bat` to create a sln file for Visual Studio 2022 with CMake. Or you can execute these cmd lines:
...
### First steps
Mist Engine uses its own implementation of CVars (like Doom) to set up and configure the engine. These variables can be set by command line args, in runtime by the console in-engine and with the `*.cfg` file. Some of them only can be set by cmd, like `workspace`. 
By default, the engine will find `default.cfg` on the root of the workspace folder. Here is an example of a cmd line to launch the engine:
...
By the default the `workspace` is set to `../../../assets/` folder, so the project setup is ready to be executed without set the `workspace` from Visual Studio. However, if you want to run the exe file manually or by cmd line, you'll have to specify the path to the asset directory.
...
On launch, the engine will load a scene file specified by `g_LoadScene` CVar. This is a `.yaml` file with the scene hierarchy, gltf models and more info (see `./assets/scenes/scene.yaml`). The file `default.cfg` contains several examples of scenes that can be loaded.
...
* Improvements on shader uniform buffer management.
...
* Forward lighting+.
* Forward lighting pipeline for alpha materials (currently WIP).
* Material system (currently WIP).
* TAA (currently WIP).
* DX12 backend.
* HBAO.
* SSR.
